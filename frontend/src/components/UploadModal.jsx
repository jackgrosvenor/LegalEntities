...
export default function UploadModal({ onClose, onUploadSuccess }) {
  ...
  const handleUpload = async () => {
    ...
    formData.append('entity_master', masterFile);
    formData.append('entity_relations', relationsFile);
    ...
    try {
      ...
      if (res.data.success) {
        setResult({
          success: true,
          message: `Uploaded ${res.data.entities_count} entities, ${res.data.relations_count} relations across ${res.data.funds_count} funds.`,
        });
        onUploadSuccess();
      } else {
        setResult({ success: false, message: res.data.error || 'Upload failed.' });
      }
    } catch (err) {
      setResult({
        success: false,
        message: err.response?.data?.error || err.message || 'Upload failed. Check file format.',
      });
    } finally {
      setUploading(false);
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center" data-testid="upload-modal">
      ...
      <div className="relative bg-white border-2 border-neutral-900 shadow-[8px_8px_0px_0px_rgba(9,9,11,1)] w-full max-w-md mx-4">
        ...
        <div className="flex items-center justify-between p-4 border-b-2 border-neutral-900 bg-neutral-900 text-white">
          <div>
            <h2 className="text-base font-bold font-heading tracking-tight">Upload New Dataset</h2>
        ...
        <div className="p-4 space-y-3">
          <FileSlot
            ...
            inputRef={masterRef}
            testId="upload-entity-master"
          />
          <FileSlot
            label="Entity Relations"
            file={relationsFile}
            onSelect={setRelationsFile}
            inputRef={relationsRef}
            testId="upload-entity-relations"
          />

          {/* Info box */}
          <div className="border border-neutral-200 bg-neutral-50 p-3">
            <p className="text-[10px] font-mono text-neutral-500 leading-relaxed">
              <span className="font-bold text-neutral-700">Entity Master</span> must include columns: ENTITY_ID, COMPANY_NAME.
              <br />
              <span className="font-bold text-neutral-700">Entity Relations</span> must include columns: PARENT_ID, CHILD_ID.
              <br />
          ...
          {/* Result message */}
          {result && (
            <div className={`border-2 p-3 flex items-start gap-2 ${result.success ? 'border-green-600 bg-green-50' : 'border-red-600 bg-red-50'}`}
            ...
            >
              ...
              <p className={`text-xs font-mono ${result.success ? 'text-green-800' : 'text-red-800'}`}>
                {result.message}
              </p>
            </div>
          )}
