...
export default function Sidebar({ selectedFundId, onFundSelect, onEntityClick, onUploadClick, dataVersion }) {
  ...
  const [filters, setFilters] = useState({ jurisdictions: [], entity_types: [] });
  ...
  useEffect(() => {
    ...
    if (selectedFundId) params.set('fund_id', selectedFundId);
    params.set('limit', '50');

    axios.get(`${API}/entities?${params.toString()}`)
      .then(res => {
  ...
  return (
    <div className="w-80 flex-shrink-0 bg-white border-r-2 border-neutral-900 h-full flex flex-col z-20" data-testid="sidebar">
      ...
      <div className="p-4 border-b border-neutral-200">
        <label className="text-[10px] font-mono uppercase tracking-[0.15em] text-neutral-500 block mb-1.5">
          Select Fund
        </label>
        <Select
          value={selectedFundId ? String(selectedFundId) : undefined}
          onValueChange={(val) => onFundSelect(val ? Number(val) : null)}
        >
          <SelectTrigger
            data-testid="fund-selector"
            className="border-2 border-neutral-300 rounded-none px-3 py-2 text-sm focus:border-neutral-900 focus:ring-0 w-full font-mono bg-white h-10"
          >
          ...
          <SelectContent className="rounded-none border-2 border-neutral-900 font-mono max-h-60">
            {funds.map(f => (
              <SelectItem
                key={f.fund_id}
                value={String(f.fund_id)}
                className="rounded-none text-sm font-mono"
                data-testid={`fund-option-${f.fund_id}`}
              >
                {f.fund_name}
              </SelectItem>
            ))}
          ...
          <div className="mt-2 space-y-2 pb-2">
            <Select value={jurisdiction || undefined} onValueChange={v => setJurisdiction(v === '_all' ? '' : v)}>
              <SelectTrigger
                ...
                className="border-2 border-neutral-300 rounded-none px-3 py-1.5 text-xs focus:border-neutral-900 focus:ring-0 w-full font-mono bg-white h-8"
              >
              ...
              <SelectContent className="rounded-none border-2 border-neutral-900 font-mono max-h-48">
                ...
                {filters.jurisdictions.map(j => (
                  <SelectItem key={j} value={j} className="rounded-none text-xs font-mono">{j}</SelectItem>
                ))}
              </SelectContent>
            </Select>
            ...
            <Select value={entityType || undefined} onValueChange={v => setEntityType(v === '_all' ? '' : v)}>
              ...
              <SelectContent className="rounded-none border-2 border-neutral-900 font-mono">
                ...
                {filters.entity_types.map(t => (
                  <SelectItem key={t} value={t} className="rounded-none text-xs font-mono">{t}</SelectItem>
                ))}
              </SelectContent>
            </Select>
      ...
      <div className="flex-1 min-h-0">
        ...
        <ScrollArea className="h-[calc(100%-32px)]">
          <div className="px-2 pb-4">
            {loading ? (
              <div className="px-2 py-8 text-center text-xs font-mono text-neutral-400">Loading...</div>
            ) : entities.length === 0 ? (
              ...
              entities.map(entity => (
                <button
                  ...
                  data-testid={`entity-list-item-${entity.ENTITY_ID}`}
                ...
                >
                  <p className="text-xs font-mono font-bold text-neutral-900 truncate group-hover:text-blue-700 transition-colors">
                    {entity.COMPANY_NAME}
                  </p>
